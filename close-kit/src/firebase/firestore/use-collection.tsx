...
export function useCollection<T = any>(
  ...
  useEffect(() => {
    ...
    const queryWithConverter = memoizedTargetRefOrQuery.withConverter(converter<T>());

    const unsubscribe = onSnapshot(
      queryWithConverter,
      (snapshot: QuerySnapshot<DocumentData>) => {
        const results: WithId<T>[] = snapshot.docs.map(doc => doc.data() as WithId<T>);
        setData(results);
        setError(null);
        setIsLoading(false);
      },
    ...
    return () => unsubscribe();
  }, [memoizedTargetRefOrQuery, context?.areServicesAvailable]);
